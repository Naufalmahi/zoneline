<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto">
        ...
        <x-card>
            <h1 class="text-xl font-bold font-heading text-gray-900 mb-6">Edit Layanan: {{ $service->name }}</h1>
            
            <form action="{{ route('owner.services.update', $service->id) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')
                
                <div>
                    <x-label for="name">Nama Layanan <span class="text-danger">*</span></x-label>
                    <x-input id="name" name="name" type="text" value="{{ old('name', $service->name) }}" required />
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <div class="flex mt-1">
                            ...
                            <input id="price" name="price" type="number" min="0" value="{{ old('price', $service->currentPrice->price ?? 0) }}" required class="flex-1 rounded-r-lg border-gray-300 shadow-sm focus:border-primary focus:ring focus:ring-primary/20">
                        </div>
                        <p class="text-xs text-gray-500 mt-1">Mengubah harga akan otomatis membuat riwayat harga baru.</p>
                    ...
                    <div>
                        ...
                        <select name="unit_type" id="unit_type" class="mt-1 w-full rounded-lg border-gray-300 shadow-sm focus:border-primary focus:ring focus:ring-primary/20">
                            <option value="kg" {{ old('unit_type', $service->unit_type) == 'kg' ? 'selected' : '' }}>Per Kilogram (kg)</option>
                            <option value="pcs" {{ old('unit_type', $service->unit_type) == 'pcs' ? 'selected' : '' }}>Per Satuan (pcs)</option>
                            <option value="meter" {{ old('unit_type', $service->unit_type) == 'meter' ? 'selected' : '' }}>Per Meter (m)</option>
                        </select>
                ...
                <div>
                    <x-label class="inline-flex items-center gap-2 cursor-pointer mt-2">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $service->is_active) ? 'checked' : '' }} class="rounded border-gray-300 text-primary focus:ring-primary">
                ...
                <div class="pt-4 border-t border-gray-100 flex justify-end gap-3">
                    ...
                    <x-button type="submit" variant="primary">Update Layanan</x-button>
                </div>
            </form>
        </x-card>
    </div>
